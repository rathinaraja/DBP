...
public class MR_App_Master{
	...
	public MR_App_Master(Workload_Parameters wp,VM_Parameters vm,int workload_num)throws IOException{
		...
		this.vm=vm; 
		vm_flav_map=new int[vm.num_flavors][wp.Num_Workloads]; 
	...
	public void start_job_threads()throws IOException{ 
		...
		while(map_flag){
			...
			if(Map_status.length==map_count){
				...
				for(int u=0;u<vm.num_flavors;u++){
					System.out.print("\t\t\t");
					for(int v=0;v<wp.Num_Workloads;v++)
						System.out.print(vm_flav_map[u][v]+"\t");
					System.out.println("");
				}
				map_flag=false; 
				reduce_flag=true; 
	...
	public void VM_flavour_map_task(){
		for(int i=0;i<Map_status.length;i++)
			vm_flav_map[(int) (Map_status[i][8]-1)][workload_num]++;
	}
}
